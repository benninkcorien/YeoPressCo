  ...
  for (var file in files) {
    ...
    downloadLanguageFile(language, file, "mo", function (err, res) {
      ...
      var p = path.join(
        contentDir,
        "languages",
        util.format(files[file], language)
      );
...
function downloadLanguageFile(language, file, format, callback) {
  ...
  if (file && file != "") {
    var url = util.format(
      "/projects/wp/dev/%s/%s/default/export-translations?format=%s",
      file,
      language,
      format
    );
    ...
    var url = util.format(
      "/projects/wp/dev/%s/default/export-translations?format=%s",
      language,
      format
    );
  }
...
function getCurrentVersion(callback) {
  var latestVersion = "6.0";
  require("simple-git")().listRemote(
    "--tags " + wordpressRepo,
    function (err, tagsList) {
      if (err) return callback(err, latestVersion);
      tagList = ("" + tagsList).split("\n");
      tagList.pop();
      lastTag = /\d\.\d(\.\d)?/gi.exec(tagList.pop());
      if (lastTag !== null) {
...
function getDbCredentials() {
  ...
  loadConfig()
    .on("done", function (contents) {
      var db = {};
      db.name = contents.match(
        /define\(["']DB_NAME["'],[\s]*["'](.*)["']\)/
      )[1];
      db.user = contents.match(
        /define\(["']DB_USER["'],[\s]*["'](.*)["']\)/
      )[1];
      db.pass = contents.match(
        /define\(["']DB_PASSWORD["'],[\s]*["'](.*)["']\)/
      )[1];
      db.host = contents.match(
        /define\(["']DB_HOST["'],[\s]*["'](.*)["']\)/
      )[1];
  ...
  getDbCredentials()
    .on("done", function (db) {
      ...
      connection.connect(function (err) {
        ...
        connection.query(
          "CREATE DATABASE IF NOT EXISTS " + mysql.escapeId(db.name),
  ...
  if (config.themeType == "git") {
    generator.remote(
      ...
      function (err, remote) {
        remote.directory(
          ".",
          path.join(config.contentDir, "themes", config.themeDir)
        );
        done();
      },
...
function setupTheme(generator, config, done) {
  ...
  if (config.themeTaskRunner == "grunt") {
    (themeTaskFile = "Gruntfile.js"), (themeTaskCmd = "grunt");
  } else if (config.themeTaskRunner == "gulp") {
    (themeTaskFile = "gulpfile.js"), (themeTaskCmd = "gulp");
  }
